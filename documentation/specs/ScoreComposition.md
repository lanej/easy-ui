...
Three columns at content widths of at least 48rem; stacked reading order in narrower containers. Exact data in expanded columns remains visible at both sizes. Source labels are visible when stacked and available to assistive technology in three columns. Independently placed contribution cards retain visible sources. No floating SVG text, chart engine, React Flow, drag handles, or zoom controls. Curves stay in gutters and follow resized nodes and opened explanations, including right-to-left layouts.
...
Signals and Contributions headers collapse their entire nonempty column into a labeled handle with a record count. The result stays visible. Controlled `collapsedColumns`, `defaultCollapsedColumns`, and `onCollapsedColumnsChange` support application-managed or local state. Hidden content stays mounted to retain node disclosures and application state, leaves keyboard/assistive-technology navigation, and restores focus to the header if necessary. Empty states retain plain headings. Collapsed tracks free space for visible columns; narrow layouts fold vertically. Paths only connect visible nodes; hidden signals reveal contribution source labels, and hidden contributions never create signal → result bypass edges. Localize action labels with `collapse` and `expand`.

Verify column keyboard control, controlled focus recovery, nested state preservation, hidden-node connector exclusion, and keyboard disclosure, stable state through record reordering, optional/external headings, shared signals, empty and missing data, explicit signal sentiment and localization, SSR, observer cleanup, container resizing, narrow widths, large text, theme changes, forced colors, browser console and accessibility scans. Signal tests must cover neutral numeric/boolean defaults, identical values with different supplied meanings, status updates, and unavailable/invalid values clearing stale interpretations. Contribution tests must distinguish the same score against different caps, exact-full and near-boundary values, independently supplied sentiment, localized fullness labels, and unavailable/outside-scale transitions clearing stale status and tint. Result tests must verify decision-before-score reading order, explicit positive/negative/neutral interpretation of the same numeric score, and independent outcomes when scores are unavailable. Browser checks must confirm signal borders and padding and sample every SVG curve against visible text rectangles to catch connector/text collisions. Verify the actual packed entry in both package layouts and both supported TypeScript resolution modes.

The [View Rule contract](../../.ui-review/README.md#score-composition-contract) covers default, expanded/larger-text, and dark states at desktop, 320px, and 4K. Separate collapsed-column states use larger text at all three sizes and require the visible nodes and both column controls. The fully expanded comparison retains all seven example nodes; source labels accompany the stacked layout. Browser audits additionally measure connector clearance and 3:1 meter-boundary contrast. The golden warning fill uses a contrasting inset edge to preserve its color and exact width.
